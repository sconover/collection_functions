  ...
  var mainFunction = function(userFeatures) {
    ...
    var factory = (function me(features, arrayFunctions, createAcrossCF) {
      ...
      function feature(featureName) {
        var featureImplementation = getFeatureIfAvailable(featureName)
        if (featureImplementation===undefined) throw new Error("Feature '" + featureName + "' is required in order to perform this operation.")
      ...
      function iterator(collection) {
        return feature("iterator")(collection)
      }
      ...
      var concat = getFeatureIfAvailable("concat") || 
                     function() {
                       var totalCost = 0
                       var newCollection = feature("newCollection")()
                       arrayFunctions.each(arguments, function(collection){
                         each(collection, function(item){feature("append")(newCollection, item)})
                         totalCost += cost
                       })
                       cost = totalCost
                       return newCollection
