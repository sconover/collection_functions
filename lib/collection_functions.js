CollectionFunctions = function(
  ...
  var factory = (function me(
    ...
    arrayCF,
    createZipCF) {
    var createZipCF = createZipCF==false ? false : true
    var cost = __nothing()
    var breaker = {}
    ...
    */
    if (arrayCF && createZipCF) {
      var zipCF = function(){
      
        var multiCollectionIterator = function(collections){
          var iteratorInstances = arrayCF.map(collections, function(collection){return newIterator(collection)})
        
          return {
            next: function() {
              return arrayCF.map(iteratorInstances, function(iterator){
                return iterator.hasNext() ? iterator.next() : __nothing()
              }, __newCollection, __collectionAppender)
            },
            hasNext: function(){
              return arrayCF.detect(iteratorInstances, function(iterator){return iterator.hasNext()})
            }
          }
        }
        ...
        var createZipCF = false //it's a way to stop recursion.  think.
        
        return me(multiCollectionIterator, __nothing, multiCollectionEquals, 
                  __newCollection, __collectionAppender, isCollectionDoesntMakeSenseWhenCollectionsAreNested, 
                  arrayCF,
                  createZipCF).functions
      }()
    }
    
    function zip() {
      
      function zipCore(collections, callback) {
      ...
      if (typeof lastArgument == "function") {
        var collections = arrayCF.slice(arguments, [0,-2])
        var callback = lastArgument
        zipCore(collections, callback)
      } else {
        var collections = arguments

        var newCollection = __newCollection()
        var callback = function() {
          var entryAsArray = arrayCF.slice(arguments, [0,-2])
          var entryAsCollection = arrayCF.map(entryAsArray, function(item){return item}, __newCollection, __collectionAppender)
          __collectionAppender(newCollection, entryAsCollection)
        }
        zipCore(collections, callback)
        return newCollection
      }
